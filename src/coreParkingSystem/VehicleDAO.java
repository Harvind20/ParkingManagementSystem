...
public class VehicleDAO implements GenericDAO<Vehicle, String> {
    ...
    public void create(Vehicle vehicle) {
        String sql = "INSERT OR IGNORE INTO vehicles(plate_num, type, is_vip, accumulated_fines) VALUES(?,?,?,0.0)";

        try (Connection conn = DatabaseConnection.connect();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            
            pstmt.setString(1, vehicle.getLicensePlate().trim().toUpperCase());
